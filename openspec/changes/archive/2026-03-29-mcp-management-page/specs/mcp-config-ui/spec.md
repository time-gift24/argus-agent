...
系统 SHALL 提供创建 MCP 配置的表单，支持 stdio/http/sse 三种 transport 类型，根据选择动态展示对应字段。
...
- **WHEN** 用户填写名称、选择 stdio transport、输入 command 后提交
- **THEN** 系统调用 `POST /api/v1/mcp-configs` 创建配置

#### Scenario: 创建配置失败（权限不足）
- **WHEN** 非 admin 用户尝试创建 stdio 类型配置
- **THEN** 系统展示后端返回的错误信息

### Requirement: 编辑 MCP 配置
系统 SHALL 允许用户编辑自己创建的配置（kind=user），修改后自动清除该配置的工具缓存。

#### Scenario: 编辑用户配置
- **WHEN** 用户点击某条 user 配置的编辑按钮
- **THEN** 打开预填当前值的编辑表单
- **AND** 提交后调用 `PATCH /api/v1/mcp-configs/{id}` 更新配置
